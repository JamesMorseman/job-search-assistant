...
**Version:** June 2026 - Phase 6 complete (all 5 packages); Phase 7 Packages 1-4 complete; Phase 7 Package 5 (Runtime Demo Hardening) implemented `9645f2a` — Leah technical CONDITIONAL PASS, Sara VISUAL FAIL; Phase 7 Package 5B (ATLAS Visual Alignment Corrective Pass) implemented `727ce71` — Leah technical/scope/privacy CONDITIONAL PASS (1019 passed/1 skipped), Sara VISUAL FAIL for reference parity (corrected reference suite); P7P5B NOT accepted as a whole, NOT screenshot-ready; Phase 7 Package 5C (ATLAS Visual Reference Alignment Corrective Pass) defined, implementation authorized; Phase 7 Package 6 screenshot capture BLOCKED; Rin-Docs BLOCKED for captured surfaces; Desktop Packages 1-11 complete
...
| 7 — Future Enhancements | **Active** — Package 1 complete `1d02117`; Package 2 complete `65cf0a7`; Package 3 complete `38314f2`; Package 4 complete `2227265`; Package 5 (Runtime Demo Hardening) implemented `9645f2a` — Leah technical CONDITIONAL PASS, Sara VISUAL FAIL; Package 5B (ATLAS Visual Alignment Corrective Pass) implemented `727ce71` — Leah technical/scope/privacy CONDITIONAL PASS (1019 passed/1 skipped), Sara VISUAL FAIL for reference parity (corrected reference suite); P7P5B NOT accepted as a whole, NOT screenshot-ready; Package 5C (ATLAS Visual Reference Alignment Corrective Pass) definition accepted, implementation authorized; Package 6 screenshot capture BLOCKED pending P7P5C + Leah/Sara re-audit; Rin-Docs BLOCKED for captured surfaces; Package 7 final release gate unauthorized |
...
### Step 2 — Prompt Phase 7 Package 5C implementation

**Phase 7 Package 5 (Runtime Demo Hardening) is implemented (commit
`9645f2a`).** Leah technical/runtime audit = CONDITIONAL PASS; Sara
visual-governance audit = VISUAL FAIL. Disposition: internal runtime evidence
only.

**Phase 7 Package 5B (ATLAS Visual Alignment Corrective Pass) is implemented
(commit `727ce71`) and re-reviewed.** Leah technical/scope/privacy re-audit =
CONDITIONAL PASS (1019 passed, 1 skipped; scope/privacy/prohibited-artifact
clean; visual-adequacy deferred to Sara). Sara visual re-audit against the
**corrected reference image suite** = VISUAL FAIL for reference parity (10
primary blockers). These are orthogonal gates and do not contradict: **P7P5B is
NOT accepted as a whole and is NOT screenshot-ready; no whole-package
acceptance is recorded.** See `DECISION_LOG.md` "Phase 7 Package 5B — Visual
Governance Reconciliation."

**Phase 7 Package 5C — ATLAS Visual Reference Alignment Corrective Pass is
defined and implementation is authorized.** This is the next implementation
step. Screenshot capture (Package 6) remains BLOCKED and Rin-Docs remains
BLOCKED for captured surfaces until P7P5C is implemented and both the Leah
technical re-audit and the Sara visual re-audit accept.

Issue Anna a compact implementation prompt. Objective: bring the ATLAS runtime
into acceptable visual parity with the corrected ATLAS reference image suite on
the surfaces intended for capture, by restoring the accepted brand/object
language and rebuilding the named surfaces — a bounded visual-reference-alignment
package, NOT a broad redesign, NOT new visual concepts, NOT a backend/feature
package.

**P0 (Sara's blocker list):** (P0-1) restore the accepted ATLAS
radar/compass/sweep brand mark (remove the generic CSS ring/dot) and the
accepted nav iconography (no bullets/dots); (P0-2) implement the Opportunity
Signal Card object and rebuild Radar as a true signal-discovery surface;
(P0-3) upgrade Opportunity Detail with full shell/header/tabs/confidence-module/
right-panel and fix the ambiguous "No" chip and the recursive "Open Opportunity
Detail" button; (P0-4) give Ask Atlas a completed-response state (Observation/
Explanation cards + populated follow-ups, fictional content); (P0-5) rebuild
Command Center to the reference layout (Action Queue, Upcoming Interviews,
Opportunity Signals row, System Health, Context Tips); (P0-6) implement the
Recommendation Card object; (P0-7) complete the global shell/topbar/footer/
mission-language system. Preserve fictional/demo-only data.

**Pipeline decision: stays excluded.** Do not rebuild the Pipeline workspace and
do not let the "Run Visibility" diagnostics surface be captured or presented as
Pipeline. Rebuilding Pipeline requires a separate authorized package.

Suggested workstreams (confirm disjoint file ownership before any parallel
issue): (1) Shell/brand/nav/mission-language — `Sidebar.tsx`, `shell.css`,
shell/brand assets (single owner, high conflict risk, do not split);
(2) Opportunity Signal Card + Radar — `Radar.tsx`, `radar.css`, new signal-card
component (after/parallel WS1 only if no shared shell files); (3) Opportunity
Detail — `OpportunityDetailSurface.tsx` + CSS; (4) Ask Atlas — `AskAtlas.tsx` +
CSS; (5) Command Center + Recommendation Card — Command Center component/CSS +
new Recommendation Card component; (6) Pipeline — excluded, no work.

Do not authorize: real/private data of any kind, public release, screenshot
capture/commits, image files, committing the untracked reference PNGs,
`docs/Artifacts/Images/*`, database file commits, seeded/fixture data commits,
credential values, Gmail/Drive content, real generated resumes/cover letters,
Rin-Docs invocation, Anna governance self-update, Tauri/Electron, installer
work, native desktop claims, production-ready / external-user-ready /
public-release-ready claims, broad redesign, new visual concepts outside the
corrected reference suite, unrelated backend features/schema changes, or
rebuilding/capturing Pipeline.

Required validation for Anna: `git status --short --untracked-files=all`,
`git diff --stat`, `git diff --name-only`, pytest, frontend build, private-data
scan, a manual runtime check of capture surfaces (Command Center, Radar,
Opportunity Detail, Ask Atlas, plus brand/shell) against the corrected
reference suite, and return of updated screenshots to Ash for Sara re-review
(Anna captures review images only; she does not commit them).

### Step 2b — Re-audit and screenshot-capture gate

After P7P5C is implemented: run the Leah technical re-audit (scope/privacy/no
prohibited artifacts) and the Sara visual re-audit (reference parity against the
corrected suite for the intended capture surfaces, Pipeline excluded). Only
after both accept may Phase 7 Package 6 (Demo Artifact Capture and Screenshot
Review) be defined and authorized, and Rin-Docs unblocked for captured surfaces.
